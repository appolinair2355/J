...
class KeepAliveSystem:
    ...
    async def wake_up_server(self):
        ...
        try:
            # Message de réveil silencieux (pas de message visible)
            logger.info("📨 Réveil serveur déclenché silencieusement")
    ...
    async def send_continuous_messages(self):
        ...
        try:
            ...
            # Mode silencieux - pas de messages visibles
            if self.message_count % 2 == 1:
                # Requêtes HTTP silencieuses pour maintenir l'activité
                logger.info(f"🔄 Maintien d'activité silencieux #{self.message_count}")

                # Faire plusieurs requêtes HTTP pour maintenir l'activité
                ...
                # Maintien d'activité silencieux côté serveur
                logger.info(f"🔄 Maintien d'activité côté serveur #{self.message_count}")
                await asyncio.sleep(2)
